...
function bowtie2_install {
    wget --no-check-certificate http://sourceforge.net/projects/bowtie-bio/files/bowtie2/2.2.4/bowtie2-2.2.4-linux-x86_64.zip
    unzip bowtie2-2.2.4-linux-x86_64.zip ; rm bowtie2-2.2.4-linux-x86_64.zip
    ln -s $PWD/bowtie2-2.2.4/bowtie2 $PWD/bin
    ln -s $PWD/bowtie2-2.2.4/bowtie2-build $PWD/bin
}
